...
$statement = $connection->prepare("SELECT * FROM `assignment1`.`user`");

//executing rows...
$statement->execute();

//selecting all the available rows from admin table...
$statement2 = $connection->prepare("SELECT * FROM `assignment1`.`admin`");
//executing rows...
$statement2->execute();
                ...
                if ($eml_exists == true){
                    ...
                    if(!$uppercase || !$lowercase || !$number || !$specialchars || strlen($password) < 6) {
                        ...
                        if ($password === $confirm_password){
                                ...
                                $folder = "./public/images/users/" . $image;

                                // moving uploaded image to specific folder...
                                move_uploaded_file($tempname, $folder);

                                // insert query for inserting data into specific columns....
                                $insert = "INSERT INTO `assignment1`.`user`(`full_name`, `username`, `email`, `password`, `user_image`) VALUES ('$full_name', '$username', '$email', sha1('$password'), '$image')";
...
// for logged in user (data update)
if(isset($_POST['updateme'])){
    ...
    if (strlen($full_name) >= 6){
        // checking if username is smaller than 3 words...
        if (strlen($username) >= 3){
            // selecting logged in user's data from admin table...
                ...
                if ($eml_exists == true){
                    ...
                    if(!$uppercase || !$lowercase || !$number || !$specialchars || strlen($password) < 6) {
                        ...
                        if ($password === $confirm_password){
                            ...
                            $update = "UPDATE `assignment1`.`user` SET `full_name`='$full_name', `username`='$username', `email`='$email', `password`=sha1('$password'), `user_image`='$image' WHERE id = '$id'";
                            $updated = $connection->query($update);
                            if ($updated){
                                $posted = true;
                                // after updating logged in user's credentials, session must be destroyed for logging in again with new credentials...
                                session_destroy();
